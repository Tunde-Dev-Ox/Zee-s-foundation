...
const MapSection = () => {
  ...
  const isInView = useInView(contentRef, { once: viewportSettings.once, amount: 0.5 });
  ...
  const mapY = useTransform(scrollYProgress, [0, 1], [120, -120]);
  const mapScale = useTransform(scrollYProgress, [0, 0.5, 1], [0.85, 1, 0.85]);
  const mapOpacity = useTransform(scrollYProgress, [0, 0.3, 0.7, 1], [0, 1, 1, 0]);

  // ✅ New shared variant for numbers
  ...
  return (
    <section
      ref={sectionRef}
      className="pt-12 overflow-hidden pb-0 relative bg-gradient-to-b from-white via-green-50/30 to-white max-[768px]:pt-0"
    >
      <motion.div
        ref={contentRef}
        ...
        className="relative z-10 max-w-[1200px] mx-auto px-5 flex flex-col items-center text-center py-[10rem] max-[768px]:py-[6rem] max-[768px]:text-left max-[768px]:items-start max-[768px]:pb-0 max-[768px]:pt-0"
      >
        {/* Number Badge */}
        <motion.div
        ...
        >
          <motion.span
            ...
            transition={{ delay: 0.6, ...baseTransition }}
          >
            5+ Countries
          </motion.span>
        </motion.div>
        ...
        >
          ...
          <motion.span
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.5, ...baseTransition }}
        ...
        {/* Animated Map */}
        <motion.div
          style={{ y: mapY, scale: mapScale, opacity: mapOpacity }}
