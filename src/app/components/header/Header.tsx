...
const Header = ({ variant = "transparent" }) => {
  ...
  return (
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all h-[75px] flex duration-300 ${isSolid ? "bg-white border-b border-gray-200" : "bg-transparent"
          }`}
      >
        <div className="flex justify-between items-center max-w-[1200px] mx-auto px-5 py-3 w-full">
          {/* Logo */}
          <div className="flex items-center">
            <Link href="/" className="flex items-center" aria-label="Zira Childcare Foundation - Return to homepage">
              {/* <Image
                src="/logo.svg"
          ...
          <nav className="block max-[860px]:hidden flex-1 justify-center">
            <ul className="flex items-center justify-center gap-6">
              {routes.map((item) => (
                <li
                  key={item.id}
                  className={`transition-colors ${isSolid
          ...
          <div className="block max-[860px]:hidden">
            <Link
              ...
              className={`inline-block cursor-pointer transition-all duration-300 ${isSolid
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.9 }}
            className={`hidden max-[860px]:block z-50 transition-colors ${isSolid ? "text-black" : "text-[#dbdbdbef]"
              }`}
